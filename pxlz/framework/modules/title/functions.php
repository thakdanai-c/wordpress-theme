...
if (!function_exists('pxlz_edgtf_include_title_types')) {
    /**
     * Load's all title types by going through all folders that are placed directly in title types folder
     */
    function pxlz_edgtf_include_title_types() {
        foreach (glob(EDGE_FRAMEWORK_MODULES_ROOT_DIR . '/title/types/*/load.php') as $module_load) {
            include_once $module_load;
        }
    }

    add_action('pxlz_edgtf_options_map', 'pxlz_edgtf_include_title_types', 1); // 1 is set to just be before title option map init
}

if (!function_exists('pxlz_edgtf_get_title')) {
    /**
     * Loads title area template
     */
    function pxlz_edgtf_get_title() {
...
if (!function_exists('pxlz_edgtf_get_title_holder_classes')) {
    /**
     * Function that adds classes to title holder div
     */
    function pxlz_edgtf_get_title_holder_classes() {
        $page_id = pxlz_edgtf_get_page_id();
        $title_type_meta = pxlz_edgtf_get_meta_field_intersect('title_area_type', $page_id);
        $title_type = !empty($title_type_meta) ? $title_type_meta : 'standard';
        $title_in_grid_meta = pxlz_edgtf_get_meta_field_intersect('title_area_in_grid', $page_id);
        $title_img = pxlz_edgtf_get_meta_field_intersect('title_area_background_image', $page_id);
        $title_img_behavior = pxlz_edgtf_get_meta_field_intersect('title_area_background_image_behavior', $page_id);
        ...
        return implode(' ', apply_filters('pxlz_edgtf_title_holder_classes', $classes));
    }
}

if (!function_exists('pxlz_edgtf_get_title_holder_styles')) {
    /**
     * Function that adds inline styles to title holder div
     */
    function pxlz_edgtf_get_title_holder_styles() {
        $page_id = pxlz_edgtf_get_page_id();
        $title_height = pxlz_edgtf_get_title_area_height();
        $title_content_padding = pxlz_edgtf_get_title_content_padding();
        $title_bg_color = pxlz_edgtf_get_meta_field_intersect('title_area_background_color', $page_id);
        $title_image = pxlz_edgtf_get_meta_field_intersect('title_area_background_image', $page_id);
        $title_image_behavior = pxlz_edgtf_get_meta_field_intersect('title_area_background_image_behavior', $page_id);
...
if (!function_exists('pxlz_edgtf_get_title_holder_data')) {
    /**
     * Function that adds data attributes to title holder div
     */
    function pxlz_edgtf_get_title_holder_data() {
        $page_id = pxlz_edgtf_get_page_id();
        $title_height = pxlz_edgtf_get_title_area_height();
        $title_img = pxlz_edgtf_get_meta_field_intersect('title_area_background_image', $page_id);
        $title_img_behavior = pxlz_edgtf_get_meta_field_intersect('title_area_background_image_behavior', $page_id);
        ...
        if (!empty($title_img) && $title_img_behavior === 'parallax-zoom-out') {
            $attachment_dimensions = pxlz_edgtf_get_image_dimensions($title_img);
        ...
        return apply_filters('pxlz_edgtf_title_holder_data', $data);
    }
}

if (!function_exists('pxlz_edgtf_get_title_wrapper_styles')) {
    /**
     * Function that adds inline styles to title wrapper div
     */
    function pxlz_edgtf_get_title_wrapper_styles() {
        $page_id = pxlz_edgtf_get_page_id();
        $title_height = pxlz_edgtf_get_title_area_height();
        $title_content_padding = pxlz_edgtf_get_title_content_padding();
        $title_img_behavior = pxlz_edgtf_get_meta_field_intersect('title_area_background_image_behavior', $page_id);
        $title_vertical_alignment = pxlz_edgtf_get_meta_field_intersect('title_area_vertical_alignment', $page_id);
...
if (!function_exists('pxlz_edgtf_get_title_background_image')) {
    /**
     * Function that return background image data if background image is set
     */
    function pxlz_edgtf_get_title_background_image() {
        $page_id = pxlz_edgtf_get_page_id();
        $title_img = pxlz_edgtf_get_meta_field_intersect('title_area_background_image', $page_id);
        $title_img_behavior = pxlz_edgtf_get_meta_field_intersect('title_area_background_image_behavior', $page_id);
        ...
        if (!empty($title_img) && $title_img_behavior !== 'hide') {
            $image_id = pxlz_edgtf_get_attachment_id_from_url($title_img);
            $alt = !empty($image_id) ? get_post_meta($image_id, '_wp_attachment_image_alt', true) : '';

            $image['src'] = $title_img;
            $image['alt'] = !empty($alt) ? esc_html($alt) : esc_html__('Image Alt', 'pxlz');
        }
...
if (!function_exists('pxlz_edgtf_get_title_area_height')) {
    /**
     * Function that returns title area height
     **/
    function pxlz_edgtf_get_title_area_height() {
        $page_id = pxlz_edgtf_get_page_id();
        $title_height_meta = pxlz_edgtf_get_meta_field_intersect('title_area_height', $page_id);
        $title_height = !empty($title_height_meta) ? intval($title_height_meta) : apply_filters('pxlz_edgtf_title_area_default_height_value', 240);

        return apply_filters('pxlz_edgtf_title_area_height', $title_height);
    }
}

if (!function_exists('pxlz_edgtf_get_title_content_padding')) {
    /**
     * Function that returns title content padding
     **/
    function pxlz_edgtf_get_title_content_padding() {
        $title_content_padding = apply_filters('pxlz_edgtf_title_content_padding', 0);

        return intval($title_content_padding);
    }
}

if (!function_exists('pxlz_edgtf_get_title_text')) {
    /**
     * Function that returns current page title text
     */
    function pxlz_edgtf_get_title_text() {
        $page_id = pxlz_edgtf_get_page_id();
...
if (!function_exists('pxlz_edgtf_get_title_styles')) {
    /**
     * Function that adds inline styles to page title
     */
    function pxlz_edgtf_get_title_styles() {
        $page_id = pxlz_edgtf_get_page_id();
...
if (!function_exists('pxlz_edgtf_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     */
    function pxlz_edgtf_subtitle_text() {
        $page_id = pxlz_edgtf_get_page_id();
...
if (!function_exists('pxlz_edgtf_get_subtitle_styles')) {
    /**
     * Function that adds inline styles to page subtitle
     */
    function pxlz_edgtf_get_subtitle_styles() {
        $page_id = pxlz_edgtf_get_page_id();
        ...
        if ($side_padding !== '') {
            if (pxlz_edgtf_string_ends_with($side_padding, '%') || pxlz_edgtf_string_ends_with($side_padding, 'px')) {
