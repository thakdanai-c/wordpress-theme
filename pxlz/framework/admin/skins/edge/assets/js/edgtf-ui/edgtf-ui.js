(function ($) {
    ...
    function edgtfChangedInput() {
        ...
        $('.edgtf-tabs-content form.edgtf_ajax_form:not(.edgtf-import-page-holder):not(.edgtf-backup-options-page-holder) #anchornav input').on('click', function () {
            ...
            setTimeout(function () {
                $('.edgtf-changes-saved').removeClass('yes');
            }, 3000);
        });
        ...
        wp.media.customlibEditGallery1 = {
            ...
            frame: function () {
                ...
                this._frame.on('update', function () {
					...
					var data = {
						action: 'pxlz_edgtf_gallery_upload_get_images',
						ids: ids,
    ...
    function edgtfInitAjaxForm() {
        ...
        $(document).delegate(".edgtf_ajax_form", "submit", function (a) {
            var b = $(this);
            var c = {
                action: "pxlz_edgtf_save_options"
            };
            jQuery.ajax({
                url: ajaxurl,
                cache: !1,
    ...
    function edgtfRepeater() {
        ...
        function initCloneRow(wrapper, button) {
            ...
            if (append) {
                            ...
                            if (thisInput.hasClass('my-color-field')) { // if input type is color picker
                                ...
                                if (typeof id !== 'undefined') {
                                    ...
                                    var textareaID = thisInput.attr('id'),
                                        ...
                                        textareaHolder = thisInput.parents('.edgtf-repeater-field').first(),
                                        idForTinyMC = id.substring('edgtf_'.length, id.length + 1),
                ...
                if (datepicker) {
                    edgtfInitDatePicker(); //init select2 script
                }
            }
            ...
            edgtfImportBtn.on('click', function (e) {
                ...
                if (confirm(confirmMessage)) {
                    ...
                    if (import_opt == 'content') {
                        for (var i = 1; i < 10; i++) {
                            var str;
                            if (i < 10) str = 'pxlz_content_0' + i + '.xml';
                            else str = 'pxlz_content_' + i + '.xml';
                            jQuery.ajax({
                                type: 'POST',
                                url: ajaxurl,
                                data: {
